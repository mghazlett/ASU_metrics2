...
forvalues i = 1/`N' {
    ...
    * 15-year, country-level
    ...
    keep if atd == 1
    cap save `tmp'_vta_`i'_c15, replace
    restore
    ...
    * 15-year, global-level
    ...
    gen ggap = rgdppc_gr - meanrgdppc_gr if atd == 1
    keep if atd == 1
    cap save `tmp'_vta_`i'_g15, replace
    restore
    ...
    * 5-year, country-level
    ...
    gen ggap = rgdppc_gr - meanrgdppc_gr if atd == 1
    keep if atd == 1
    cap save `tmp'_vta_`i'_c5, replace
    restore
    ...
    * 5-year, global-level
    ...
    gen ggap = rgdppc_gr - meanrgdppc_gr if atd == 1
    keep if atd == 1
    cap save `tmp'_vta_`i'_g5, replace
    restore
}

* Collapse all specs
foreach s in c5 c15 g5 g15 {
    cap use `tmp'_vta_1_`s', clear
    forvalues i = 2/`N' {
        cap append using `tmp'_vta_`i'_`s'
    }
    egen mggap = mean(ggap)
    collapse mggap
    gen spec = "`s'"
    save `tmp'_vxa_`s', replace
}

use `tmp'_vxa_c5, clear
append using `tmp'_vxa_c15
append using `tmp'_vxa_g5
append using `tmp'_vxa_g15
...
forvalues i = 1/200 {
    foreach s in c5 c15 g5 g15 {
        cap erase `tmp'_vta_`i'_`s'.dta
    }
}
foreach s in c5 c15 g5 g15 {
    cap erase `tmp'_vxa_`s'.dta
}
