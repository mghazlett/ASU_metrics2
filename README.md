...
To Erhan, here are the files you actually need to run my replication code: 
 ...
 ├── runall_inpaper_pkg.bat        ← run this to reproduce everything, EDIT TO MATCH VERSIONS ON YOUR MACHINE
  ├── InPaper_20260320.tex          ← LaTeX source for PDF output                     ├── README_REP_MGH.docx                                                                     ├── data/                                                                           │   └── ple_dataset.dta           ← pre-built panel (replaces ~20 raw files +       datprep.do that was in the authors' original replication code)
  ├── programs/
